...
BenchmarkResult benchmark_falcon(std::vector<KeyType> &data,
                                              ...
                                              falcon::CachePolicy policy = falcon::CachePolicy::LRU,
                                              falcon::IOInterfaceType io_iface = falcon::IO_URING,
                                              bool use_odirect = true,
                                              int workers = 1,
                                              std::vector<KeyType>* probe_points = nullptr,
    ...
    auto t0 = timer::now();
    sort(probe_points->begin(), probe_points->end());
    auto lo = probe_points->front(), hi = probe_points->back();
    auto fut = engine.range_lookup(lo, hi);
    auto rr  = fut.get();
    

    if (probe_points && has_window) {
        const auto& Q = *probe_points;
        size_t qL = 0, qR = Q.size();
        // const auto& R = rr.keys;            // ordered keys
        // size_t i = 0, j = 0;
    ...
    BenchmarkResult r;
    r.epsilon        = Epsilon;
    r.time_ns        = double(total_ns);   // 单次 query 的 latency
...
int main() {
    ...
    std::string query_filename = "books_200M_uint64_unique.1Mtable5.bin";
    ...
    for (falcon::CachePolicy policy : {falcon::CachePolicy::LRU}) {
