...
const char* DATA_FILE   = "/mnt/home/zwshi/Datasets/SOSD/fb_200M_uint64_unique";                             
...
void* worker_thread(void* arg) {
    ...
    for (size_t i = args->start; i < args->end; i++) {
        ...
        uint64_t pos = fb_rmi::lookup(q, &err);
...
int main() {
    std::string filename = "fb_200M_uint64_unique";
    std::string query_filename = "fb_200M_uint64_unique.query.bin";
    ...
    // construct index
    if (!fb_rmi::load(DATA_PATH)) {
        std::cerr << "Failed to load RMI model!" << std::endl;
        return 1;
    }

    std::ofstream ofs("fb_rmi_disk_multithread.csv");
