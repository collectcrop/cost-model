...
static int run_queries_multithread(StxDiskKV& kv,
  ...
  uint64_t total_thread_wall_ns = 0;
  uint64_t total_thread_cpu_ns  = 0;
  for (int i = 0; i < threads; i++) {
...
int main(int argc, char* argv[]) {
  // 路径按需修改
  const char* data_path  = "/mnt/home/zwshi/Datasets/SOSD/books_200M_uint64_unique";              // 主数据文件（8B/条）
  const char* query_path = "/mnt/home/zwshi/Datasets/SOSD/books_200M_uint64_unique.query.bin";    // 查询文件（8B/条）
  ...
  fprintf(csv, "threads,avg_latency_ns,avg_walltime_s,avg_iops,mem_time_s,io_time_s\n");

  for (int t = 0; t <= 0; t++) { 
    int threads = 1 << t;
    double total_latency = 0, total_time = 0, total_iops = 0;
    double total_mem_s = 0, total_io_s = 0;
