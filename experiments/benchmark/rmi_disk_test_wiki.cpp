...
const char* DATA_FILE   = "/mnt/home/zwshi/Datasets/SOSD/wiki_ts_200M_uint64_unique";                             
...
void* worker_thread(void* arg) {
    ...
    for (size_t i = args->start; i < args->end; i++) {
        ...
        uint64_t pos = wiki_rmi::lookup(q, &err);
...
int main() {
    std::string filename = "wiki_ts_200M_uint64_unique";
    std::string query_filename = "wiki_ts_200M_uint64_unique.query.bin";
    ...
    // construct index
    if (!wiki_rmi::load(DATA_PATH)) {
        std::cerr << "Failed to load RMI model!" << std::endl;
        return 1;
    }

    std::ofstream ofs("wiki_rmi_disk_multithread.csv");
