...
const char* DATA_FILE   = "/mnt/home/zwshi/Datasets/SOSD/books_200M_uint64_unique";                             
...
void* worker_thread(void* arg) {
    ...
    for (size_t i = args->start; i < args->end; i++) {
        ...
        uint64_t pos = books_rmi::lookup(q, &err);
...
int main() {
    ...
    // construct index
    if (!books_rmi::load(DATA_PATH)) {
        std::cerr << "Failed to load RMI model!" << std::endl;
        return 1;
    }

    std::ofstream ofs("books_rmi_disk_multithread.csv");
    ofs << "baseline,threads,latency_ns,walltime_s,avg_IOs,mem_time_s,io_time_s\n";
