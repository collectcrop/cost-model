...
private:
    ...
    //     return;
    // }
    void write_data(const void *data, off_t offset, size_t len) {
        const size_t ALIGN = 4096;

        off_t aligned_off = offset & ~(ALIGN - 1);  // 向下对齐
        off_t end_off     = (offset + len + ALIGN - 1) & ~(ALIGN - 1); // 向上对齐
        size_t aligned_len = end_off - aligned_off;

        void *aligned_buf = nullptr;
        if (posix_memalign(&aligned_buf, ALIGN, aligned_len) != 0) {
            perror("posix_memalign");
            return;
        }

        // 先读出原始数据
        ssize_t r = pread(fd, aligned_buf, aligned_len, aligned_off);
        if (r < 0) {
            perror("pread failed");
            free(aligned_buf);
            return;
        ...
        // 覆盖用户需要写入的部分
        memcpy((char*)aligned_buf + (offset - aligned_off), data, len);

        // 写回
        ssize_t written = 0;
        while (written < (ssize_t)aligned_len) {
            ssize_t w = pwrite(fd, (char*)aligned_buf + written, aligned_len - written, aligned_off + written);
            if (w < 0) {
    ...
    void read_block(void *data, int block_id) {
        ...
        void *aligned_buf = nullptr;
        const size_t ALIGN = 4096; // 根据文件系统调整
        if (posix_memalign(&aligned_buf, ALIGN, BLOCK_SIZE) != 0) {
            perror("posix_memalign");
            return;
        }
        ssize_t read_bytes = 0;
        while (read_bytes < (ssize_t)BLOCK_SIZE) {
            ssize_t r = pread(fd, (char*)aligned_buf + read_bytes, BLOCK_SIZE - read_bytes, offset + read_bytes);
            if (r < 0) {
                if (errno == EINTR) continue;
                perror("pread failed");
                free(aligned_buf);
                ...
                // EOF: 视为错误或填 0
                memset((char*)aligned_buf + read_bytes, 0, BLOCK_SIZE - read_bytes);
                break;
            }
            read_bytes += r;
        }
        memcpy(data, aligned_buf, BLOCK_SIZE);
        free(aligned_buf);
    }
    ...
    void write_block(const void *data, int block_id) {
        off_t offset = (off_t)block_id * BLOCK_SIZE;

        void *aligned_buf = nullptr;
        if (posix_memalign(&aligned_buf, 4096, BLOCK_SIZE) != 0) {
            perror("posix_memalign");
            return;
        }
        memcpy(aligned_buf, data, BLOCK_SIZE);

        ssize_t r = pwrite(fd, aligned_buf, BLOCK_SIZE, offset);
        if (r < 0) {
            perror("pwrite failed");
        }
        free(aligned_buf);
    ...
    //     return;
    // }
    // void read_data(void *data, long offset, size_t len) {
    //     ssize_t read_bytes = 0;
    //     while (read_bytes < (ssize_t)len) {
    //         ssize_t result = pread(fd, (char*)data + read_bytes, len - read_bytes, offset + read_bytes);
    //         if (result < 0) {
    ...
    //         read_bytes += result;
    //     }
    //     // 如果读取的字节数不足 len，用零填充剩余部分
    //     if (read_bytes < (ssize_t)len) {
    //         memset((char*)data + read_bytes, 0, len - read_bytes);
    //     }
    // }
...
public:
    ...
    void init(char *fn, bool is_first, int memory_type) {
        if (is_first) {
            // fp = fopen(fn, "wb");
            fd = open(fn, O_CREAT | O_TRUNC | O_RDWR | O_DIRECT, S_IRUSR | S_IWUSR);
            if (fd == -1) {
                perror("open failed for create");
                return;
            }
            NEXT_BLOCK = 1;
            NEXT_OFFSET = 0;
...
//                char data[BLOCK_SIZE];
    ...
    int lippb_range_entry(T lo, T hi, int *bc, int *inner_c) {
        ...
        while (true) {
            // 在当前叶子内，从 start_pos 位置开始扫描
            for (; start_pos < blfnh->item_count; ++start_pos) {
                T k = bis[start_pos].key;
                if (k < lo) {
                    continue;   // 理论上 search 已保证 >= lo，这里只是防御
                }
                if (k > hi) {
                    // 超出上界，整个 [lo,hi] 扫描完毕
                    return cnt;
            ...
            // 当前叶子扫完了，如果没有下一个叶子，则提前结束
            if (blfnh->next_block_id == -1) {
                return cnt;
            }
            ...
            *bc += 1;
            start_pos = 0;
            blfnh = (BLeafNodeHeader *)start_block;
            bis   = (BItem *)(start_block + BLeafNodeHeaderSize);
        }
    }
